...
| `public` | boolean | On a private site, allow this page without authentication. |
| `groups` | string array | Limit the page to matching reader groups. On a private OAuth or JWT site this **restricts access**. With [shared session](/docs/writing/personalization) it **hides the page from navigation** only. |
...
See [Authentication setup](/docs/deploy/authentication-setup) for `public` and
`groups` on private sites. See [Personalized content](/docs/writing/personalization)
for `groups` with shared session.
