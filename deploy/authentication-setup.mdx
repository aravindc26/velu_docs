...
To identify visitors on a **public** site without locking pages, use
[shared session personalization](/docs/writing/personalization) instead.
Shared session and private authentication are mutually exclusive.
...
group-restricted pages. On a public site with
[shared session](/docs/writing/personalization), `groups` hide pages from
navigation only.
...
| `content` | no | Object exposed as MDX `user`. See [Personalized content](/docs/writing/personalization). |
...
<CardGroup>
  <Card title="Personalized content" icon="user" href="/docs/writing/personalization">
    Shared session, MDX `{user}` fields, and OpenAPI group filtering on a public site.
  </Card>
